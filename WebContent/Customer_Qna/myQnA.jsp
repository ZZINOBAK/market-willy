...
<%
	...
	List<QnaVO> list = QnaDAO.viewId(vo.getId());
...
<link rel="stylesheet" href="../resource/css/QnA.css" />
    ...
    <tbody id="tbody">
      ...
      <c:otherwise>
      	<c:forEach var="vo" items="${list }">
      		<tr>
      			<td class="centers">${vo.qnaNum }</td>
      			<td>
      			<!-- 게시물 클릭시 비밀번호 뜨게 id 설정 -->
      			<a href="view.jsp?qnaNum=${vo.qnaNum }&id=${vo.id}">${vo.qnaTitle }<a>
      			</td>
      			<td class="centers">${vo.id }</td>
      			<td class="centers">${vo.qnaDate }</td>
      			<td class="centers">${vo.qnaInfo }</td>
      		</tr>
      	</c:forEach>
      </c:otherwise>
      </c:choose>
			...
			<tr>
				<td colspan="3">
					<ol class="paging">
						...
						<c:if test="${pvo.beginPage != 1 }">
						<li>
							<a href="qnA.jsp?cPage=${pvo.beginPage - 1 }">이전으로</a>
						</li>
						</c:if>
						...
						<c:if test="${pageNo != pvo.nowPage }">
							<li>
								<a href="qnA.jsp?cPage=${pageNo }">${pageNo }</a>
							</li>
						</c:if>
						</c:forEach>
						...
						<c:if test="${pvo.endPage < pvo.totalPage }">
							<li>
								<a href="qnA.jsp?cPage=${pvo.endPage + 1}">다음으로</a>
							</li>
						</c:if>
					</ol>
				</td>
				<td>
						<a href="writeQna.jsp" class="write_btn"> 문의글 작성</a>
				</td>
			</tr>
		</tfoot> --%>
  </table>
